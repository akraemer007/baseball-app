...
Also produce a short headline-voice **title** that captures the throughline of the bullets. Same voice as a recap headline: factual, concrete, no hype. 4 to 7 words. Title-case team and player names; everything else lowercase. No punctuation at the end.
...
- `Recent Cubs trends` (vague)
- `It's been a busy week!` (hype, exclamation)
- `Could the Cubs be heating up?` (rhetorical question, hedge)
- `Cubs win 8 of 9 then lose three in a row to fall to 17-12 entering the weekend` (way too long)

If the bullets contradict each other, pick the most recent thread. If the input is too thin for a meaningful title, return `Two-week summary` as a fallback.
...
`title` is the headline described above. `text` is the bullet sentence. `metric_ref` is a short tag (≤ 30 chars) naming the input field or game the bullet leans on — e.g. `rolling_batting_14d:PCA`, `team_day_last_14:run_diff`, `recap:2026-04-22`, `milestone:team_winning_streak`. The app uses `metric_ref` for hover/debug; it doesn't render it as prose.
